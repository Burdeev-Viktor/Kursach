...
public class LessonsController implements Initializable {
    ...
    public void setInfo(){
        List<Lesson> lessons = lessonService.findAllByIdUser(Session.getId());
        ...
        butAdd.setOnAction(event -> {
            ...
            if(lessonService.lessonIsExistsByNameAndIdUser(twName.getText(), Session.getId())){
                Alert alert = new Alert(Alert.AlertType.ERROR);
        ...
        checkBox.setOnAction(event -> twCountLab.setDisable(!checkBox.isSelected()));
